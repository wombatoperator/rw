...
export default function FileViewerPage() {
  ...
  return (
    <main className="flex flex-1 flex-col px-4 sm:px-6 py-6 sm:py-12 max-w-6xl mx-auto w-full">
      <div className="flex flex-wrap gap-1.5 sm:gap-2 text-[10px] sm:text-xs text-white/30 mb-6 sm:mb-8" style={{ fontFamily: "var(--font-mono)" }}>
      ...
      <div className="flex flex-wrap gap-2 sm:gap-4 text-[10px] sm:text-xs text-white/30 mb-6 sm:mb-8" style={{ fontFamily: "var(--font-mono)" }}>
        ...
        <span className="text-white/15 hidden sm:inline">{file.filename}</span>
      </div>
      ...
      <div className="flex justify-between items-start gap-4" style={{ fontFamily: "var(--font-mono)" }}>
        {prevFile ? (
          <Link href={`/browse/${params.category}/${prevFile.id}`} className="text-[10px] sm:text-xs text-white/40 hover:text-dmt-purple active:text-dmt-purple transition-colors min-h-[44px] flex items-center">
            &larr; {prevFile.name.slice(0, 20)}{prevFile.name.length > 20 ? "..." : ""}
          </Link>
        ) : <div />}
        {nextFile ? (
          <Link href={`/browse/${params.category}/${nextFile.id}`} className="text-[10px] sm:text-xs text-white/40 hover:text-dmt-cyan active:text-dmt-cyan transition-colors text-right min-h-[44px] flex items-center">
